...
- **Lixeira — visualizar (`loadDeletedAthletes`):** a tela de listagem `AdminAthletes` exibe um `Switch` "Mostrar apenas os deletados" no header. Quando ativado, chama `loadDeletedAthletes` (IPC `load-deleted-athletes`) que retorna apenas itens com `deletedAt != null`, ordenados alfabeticamente por nome. Botões "Importar", "Exportar JSON" e "Cadastrar Atleta" são ocultados na view deletados (apenas restauração/exclusão permanente são ações válidas). O título da página muda para "Atletas Deletados" e o dashboard de estatísticas (Inscritos + Graduações) é ocultado.
...
- **Lixeira — visualizar (`loadDeletedArbitros`):** a tela de listagem `AdminArbitros` exibe um `Switch` "Mostrar apenas os deletados" no header. Quando ativado, chama `loadDeletedArbitros` (IPC `load-deleted-arbitros`) que retorna apenas itens com `deletedAt != null`, ordenados alfabeticamente por nome. Botões "Importar", "Exportar" e "Cadastrar" são ocultados na view deletados. O título da página muda para "Árbitros Deletados".
...
- **Lixeira — visualizar (`loadDeletedAreas`):** a tela de listagem `AdminAreas` exibe um `Switch` "Mostrar apenas os deletados" no header. Quando ativado, chama `loadDeletedAreas` (IPC `load-deleted-areas`) que retorna apenas itens com `deletedAt != null`, ordenados alfabeticamente por nome. Botões "Importar", "Exportar JSON" e "Cadastrar" são ocultados na view deletados. O título da página muda para "Áreas de Luta Deletadas".
