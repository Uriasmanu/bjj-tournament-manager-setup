...
- **O que é:** Adição de campo de busca textual nas telas de Chaves, Atletas, Árbitros e Equipes, permitindo filtrar os registros exibidos por nome/texto. Correção do bug que acumula `chaveIds` nos árbitros ao regenerar chaves.
...
- **Quem usa:** Administradores do torneio que gerenciam chaves, atletas, árbitros e equipes.
- **Escopo:** Adição de `TextInput` com `search` prop do Mantine nas 4 telas + correção do acúmulo de `chaveIds` em `autoAtribuirArbitros`.
...
- [ ] RF-05: Ao regenerar chaves, os `chaveIds` de todos os árbitros devem ser limpos antes da reatribuição automática
- [ ] RF-06: A atribuição manual de árbitro (`atribuirArbitroHandler`) deve continuar funcionando corretamente
...
- [ ] CA-05: Dado um torneio com chaves geradas e árbitros atribuídos, quando clicar em "Gerar Novamente", então o `chaveIds` de cada árbitro deve conter apenas IDs de chaves que existem atualmente
